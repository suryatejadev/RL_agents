...
class Agent_QLearn:
    ...
    def get_action(self,state,method):
        ...
        # Method='eps_greedy' is used to sample an action from present state
        ...
        if method=='eps_greedy':
            ...
            # that gives maximum q_func value
                ...
                fourier_cache = [feat_vector,q_func[action_index],action_index]
                return self.action[action_index],fourier_cache
        # Method='max' is used to get the maximum q value from the next state
        # This returns the maximum q value
        elif method=='max':
            return q_func.max()
    ...
    def update(self,present_val,reward,next_val=None):
        ...
        if next_val==None:
            ...
            td_error = reward + self.gamma*next_val - present_val[1]
        self.weights[present_val[2],:] += self.alpha * td_error * present_val[0]
        return

def Run_QLearn_trial(parameters_index):
    ...
    agent = Agent_QLearn(alpha=alpha, gamma=gamma,
    ...
    for iter_episode in range(num_episodes):
        ...
        for iter_time in range(num_timesteps):
            ...
            present_action,present_cache = agent.get_action(state=env.normalize(present_state),method='eps_greedy')
            ...
            if env.terminal_state==True:
                ...
                # Return the maximum value from the next state.
                next_cache = agent.get_action(state=env.normalize(next_state),method='max')
                # Update weights of the agent
...
def Run_QLearn_parallel(parameters):

    num_trials = parameters['num_trials']
    num_episodes = parameters['num_episodes']
    returns_list = []
    p = Pool()
    returns_list.append(p.map(Run_QLearn_trial,itertools.izip(range(num_trials),itertools.repeat(parameters))))
